...
public class ClientsService : IClientsService
{
    ...
    public async Task<List<TripForClientDTO>> GetTripsByIdAsync(int id, CancellationToken cancellationToken)
    {
        ...
        string command =
            @"Select t.IdTrip, t.Name AS TripName, t.Description, t.DateFrom, t.DateTo, t.MaxPeople, C.Name AS CountryName, CT2.PaymentDate, CT2.RegisteredAt from Trip t 
    ...
    public async Task<int> AddTripToClient(int id, int tripId, CancellationToken cancellationToken)
    {
        ...
        string checkPeopleCommand =
            "SELECT COUNT(*) FROM Client join dbo.Client_Trip CT on Client.IdClient = CT.IdClient where Ct.IdTrip = @TripId";
        string insertCommand =
            "insert into Client_Trip (IdClient, IdTrip, RegisteredAt, PaymentDate) values (@id,@tripId,@registeredAt,@PaymentDate)";
        using (SqlConnection conn = new SqlConnection(_connectionString))
        using (SqlCommand cmd = new SqlCommand(checkClientCommand, conn))
        {
            ...
            cmd.CommandText = checkTripCommand;
            cmd.Parameters.AddWithValue("@id", id);
            cmd.Parameters.AddWithValue("@tripId", tripId);
            ...
            cmd.Parameters.Clear();
            cmd.CommandText = checkPeopleCommand;
            cmd.Parameters.AddWithValue("@tripId", tripId);
            int people = -1;
            using (var reader = await cmd.ExecuteReaderAsync(cancellationToken))
            {
